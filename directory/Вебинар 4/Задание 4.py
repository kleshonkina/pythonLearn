...
def multiplication_chain(num):
    ...
    # Отсекаем последний вариант из примера (условия задачи).
    if num > 9:
        curr_num = num

        # Пока результат умножения цифр числа больше 1 цифры, будем умножать.
        while len(str(curr_num)) > 1:
            # Результат произведения цифр числа.
            our_multiply = 1

            for digit in str(curr_num):
                our_multiply *= int(digit)

            curr_num = our_multiply

            # Увеличиваем количество итераций произведений на 1.
            count_multy += 1
