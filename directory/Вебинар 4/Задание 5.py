...
def josephus_task(num_people, kill_num):
    # Второй способ решения по ссылке из википедии.
    survivors = [True] * num_people
    count_survivors = num_people
    survivor, index = 0, 0

    while count_survivors:
        # Если воин живой.
        if survivors[survivor]:
            index += 1

            # Проверяем, нужный ли это порядковый номер, чтоб убить.
            if index == kill_num:
                # Убавляем количество живых.
                count_survivors -= 1
                if not count_survivors:
                    break
                else:
                    survivors[survivor] = False
                    index = 0
