...
def scrabble(word):
    """
    Метод рассчитывает количество очков для входящей строки, учитывая, что
    для определенной буквы полагается определенное количество очков.
    :param word: Строка для подсчета.
    :return: Результирующее количество очков.
    """
    char_points = {
        1: 'авеёинорст',
        2: 'дклмпу',
    ...
    points = 0
    for char in word:
        for cur_points, chars in char_points.items():
            if char in chars:
                points += cur_points
    return points
