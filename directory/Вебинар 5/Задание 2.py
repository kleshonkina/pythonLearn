...
def repeats(our_str):
    """
    Метод модифицирует входящую строку, добавляя постфикс '_n' к каждому символу,
    где n — число, означающее, в какой раз по счету мы встречаем текущий символ в строке.
    :param our_str: Строка текста.
    :return: Строка с подфиксами _n, где n — сколько раз эта буква уже встретилась в строке.
    """
    repeats_stat = {}
    new_str = ''
    for char in our_str:
        repeats_stat[char] = 1 if not repeats_stat.get(char) else repeats_stat.get(char) + 1
        new_str += f'{char}_{repeats_stat[char]}'
    return new_str
