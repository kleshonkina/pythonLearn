...
def everything_for_your_cat(cats_data):
    """
    Метод находит всех животных одного владельца, формируя строку
    вида: 'Имя владельца': 'Все его животные'.
    :param cats_data: Информация о животных и их владельцах.
    :return: Результирующая строка со списком животных по каждому владельцу.
    """
    owners_data = {}
    our_str = ''
    for info in cats_data:
        current_dict_key = f'{info[-2]} {info[-1]}'
        current_cat_info = f'{info[0]}, {info[1]}'
        if not owners_data.get(current_dict_key):
            owners_data[current_dict_key] = current_cat_info
        else:
            owners_data[current_dict_key] = f'{owners_data.get(current_dict_key)}; {current_cat_info}'
    for owner_name, cat_info in owners_data.items():
        our_str += f'{owner_name}: {cat_info}\n'
    return our_str
