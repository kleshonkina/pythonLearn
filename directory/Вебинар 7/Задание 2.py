...
class PersonInfo:
    def __init__(self, name: str, age: int, *divisions):
        """
        Конструктор класса PersonInfo. Принимает Фамилию и Имя, возраст сотрудника и подразделения.
        :param name: Строка вида "Имя Фамилия" сотрудника.
        :param age: Возраст сотрудника.
        :param divisions: Подразделения.
        """
        self._name = name
        self._age = age
    ...
    def short_name(self) -> str:
        """
        Получить строку вида "Фамилия И".
        :return: Возвращает строку Фамилия И.
        """
        return f"{self._name.split(' ')[-1]} {self._name.split(' ')[0][0]}."

    def path_deps(self) -> str:
        """
        Метод возвращает строку "Головное подразделение --> Подразделение --> Конечное подразделение".
        :return: Строка с подразделениями, разделенными через "-->".
        """
        return ' --> '.join(self._divisions)

    def new_salary(self) -> int:
        """
        Метод возвращает проиндексированную зарплату.
        :return: Сумма зарплаты после индексации.
        """
        deps_str = ''.join(self._divisions)
        deps_set = set(deps_str)
        counts_symbols = []
        for symbol in deps_set:
            counts_symbols.append(deps_str.count(symbol))
        counts_symbols.sort()
        sum_count_symbols = sum(counts_symbols[-3:])
        return 1337 * self._age * sum_count_symbols
