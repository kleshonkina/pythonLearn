...
class RomanNums:
    def __init__(self, roman_num: str):
        """
        Конструктор класса RomanNums принимает строку римского числа.
        :param roman_num: Строка римского числа
        """
        self.roman_num = roman_num

    def from_roman(self):
        """
        Метод переводит римскую запись числа в арабскую.cdtnkfyf
        :return: Число, переведённое из римского числа в арабское.
        """
        result = 0
        rim_to_arab = {
            'I': 1,
        ...
        for i in range(len(self.roman_num) - 1):
            if rim_to_arab[self.roman_num[i]] < rim_to_arab[self.roman_num[i + 1]]:
                result -= rim_to_arab[self.roman_num[i]]
            elif rim_to_arab[self.roman_num[i]] >= rim_to_arab[self.roman_num[i + 1]]:
                result += rim_to_arab[self.roman_num[i]]
        return result + rim_to_arab[self.roman_num[len(self.roman_num) - 1]]

    def is_palindrome(self):
        """
        Проверить число на палиндром.
        :return: Является ли число палиндромом.
        """
